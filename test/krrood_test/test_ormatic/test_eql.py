...
def test_set_of_transitive_attributes(session):
    """
    Verify that set_of with transitive attributes generates a JOIN to
    GraspDescriptionDAO.
    """
...
def test_set_of_move_action_transitive(session):
    """
    Verify that set_of with both direct and transitive attributes generates
    correct JOINs.
...
def test_set_of_same_table_twice(session):
    """
    Verify that two variables of the same type produce separate JOINs with
    aliases.
...
def test_case_when_with_min(session):
    ...
    THEN ... END). Simulates: MIN(CASE WHEN
    d.polymorphic_type='PickUpActionDAO' THEN id END) from the big plan
    query.
    """
...
def test_entity_from_multi_hop_attribute(session, database):
    ...
    AttributeResolutionError because _translate_entity_from_attribute looks up
    the outermost attribute name ('position') directly on the leaf DAO
    (NestedActionDAO) instead of walking the full chain.

    After the fix the translator must join through KRROODPoseDAO and
    return KRROODPositionDAO instances for all linked actions.
    """
...
def test_entity_with_relationship_selected_variable(session, database):
    """
    Prove bug: entity(m.grasp_config).where(m.robot_x > 0.5) produces a cross-
    join instead of an INNER JOIN, returning too many results.

    After the fix, GraspConfigDAO must be joined with MoveActionDAO via
    the FK so only the grasp_config linked to a high-robot_x move is
    returned.
    """
...
def test_count_all_derives_dao_from_where_clause(session, database):
    """
    Prove bug: set_of(count_all()).where(m.robot_x > 0.5) raises
    NoDAOFoundError because CountAll carries no DAO information.

    After the fix, the translator must fall back to the WHERE clause to
    find the base DAO (MoveActionDAO via m.robot_x) and emit SELECT
    count(*) FROM MoveActionDAO.
    """
...
def test_order_by_aggregate(session, database):
    """
    Prove bug: set_of(b.size, c).grouped_by(b.size).ordered_by(c,
    descending=True) raises an error because _apply_clauses calls
    translate_attribute() on a Count aggregator instead of
    _translate_comparator_operand().

    After the fix, the translator must emit: SELECT size, COUNT(*) FROM
    BodyDAO GROUP BY size ORDER BY COUNT(*) DESC and return rows ordered
    with the most-frequent size first.
    """
    session.add_all([
