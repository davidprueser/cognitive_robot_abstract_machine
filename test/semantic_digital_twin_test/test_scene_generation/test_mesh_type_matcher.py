...
def test_a_missing_type_yields_nothing_rather_than_a_wrong_mesh() -> None:
    ...
    assert matcher.random_match(ObjectType.SOFA, Scale(x=0.9, y=2.0, z=0.8)) is None


def test_the_closest_size_match_of_the_right_type_is_preferred() -> None:
    """
    The circuit samples a size for each piece, so honouring it is what keeps a sampled
    0.4 m stool from spawning as a 1.2 m armchair.
    """
    matcher = _MeshTypeMatcher(
        candidates=[
    ...
    match = matcher.random_match(
        ObjectType.CHAIR, target_extents=Scale(x=0.5, y=0.5, z=0.9)
    )
...
def test_a_candidate_wildly_off_the_sampled_size_is_rejected() -> None:
    """
    A mesh whose real size bears no relation to what was sampled makes the room look
    wrong even though its category is right, so tolerance has a limit.
    """
    matcher = _MeshTypeMatcher(
...
def test_size_is_ignored_when_no_target_is_given() -> None:
    """
    Shelf contents are matched by category alone, so omitting the target size must keep
    the old behaviour for that caller.
    """
    matcher = _MeshTypeMatcher(
        candidates=[_candidate(ObjectType.BOOK, 0.2, 0.05, 0.25, "book")]
    )
...
def test_a_candidate_of_unknown_size_stays_eligible() -> None:
    ...
    would shrink an already-thin pool for no reason.
    """
