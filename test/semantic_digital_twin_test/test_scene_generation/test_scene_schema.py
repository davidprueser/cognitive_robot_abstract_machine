...
def _make_shelf(
    ...
    """
    A shelf whose layers may deliberately disagree with it in footprint, which is
    what an independently sampled shelf looks like before the footprint is pinned.
    """
...
def _slab_heights(shelf: EGShelf) -> list[float]:
    ...
    Slabs are reparented onto the corpus so the shelf moves as one unit, which
    puts their origins in the corpus frame, centred half a shelf up.
    """
    return sorted(
        float(
...
def test_shelf_mounts_at_the_origin_of_the_parent_it_is_given(
    ...
    """
    A shelf defines the frame its contents are expressed in and sits at that
    frame's origin, so a caller positions it by choosing the parent rather than
    by a pose on the shelf. The corpus is still built in the content frame, so
    its yaw is :attr:`EGShelf.CONTENT_FRAME_YAW_OFFSET_DEGREES`, which extraction
    has to invert.
    """
    shelf = _make_shelf()
    shelf.source_ids = [
...
def test_shelf_type_survives_a_json_round_trip() -> None:
    """
    The type is what a sampled shelf is conditioned on, so a stored shelf that
    lost it would come back as a shelf of no particular kind.
    """
    shelf = _make_shelf()

    restored = EGShelf.from_json(shelf.to_json())

    assert restored.shelf_type is ShelfType.BOOKCASE
    assert [layer.shelf_type for layer in restored.layers] == [ShelfType.BOOKCASE]


def test_every_slab_spawns_at_the_shelfs_own_footprint() -> None:
    """
    Layers are drawn independently, so their footprints can disagree with each
    other and with the shelf. Spawning each at its own size leaves smaller slabs
    floating clear of the corpus walls, so the shelf's footprint is the single
    source of truth.
    """
    shelf = _make_shelf(
...
def test_the_corpus_interior_matches_the_shelfs_learned_dimensions() -> None:
    """
    The corpus used to be sized from the layers, so the shelf's own learned width
    and depth had no effect and every type spawned the same box. They are what
    distinguishes a narrow bookcase from a wide cabinet.
    """
...
def test_slabs_are_evenly_spaced_whatever_heights_were_drawn() -> None:
    ...
    an empty shelf level leaves no trace, so a measured gap is the distance to the
    next occupied level. Real shelves are evenly spaced, so the drawn heights must
    not become slab positions.
    """
...
def test_every_slab_gap_leaves_room_for_a_typical_object() -> None:
    """
    Layers drawn close together used to be pushed apart by 3 cm, which is far less
    than anything stands on a shelf, so the layer spawned empty. The median object
    in the dataset is 0.077 m tall.
    """
...
def test_objects_recorded_on_top_of_low_furniture_spawn_above_it() -> None:
    """
    A quarter of the recorded layers describe things standing *on* a piece of
    furniture, not on a shelf inside it. On a low cabinet that is where they
    belong, and spawning them inside crushes them against the corpus ceiling.
    """
    shelf = _make_shelf(
...
def test_objects_recorded_on_top_of_tall_furniture_spawn_inside_it() -> None:
    ...
    threshold the recorded top layer is an ordinary level. Its objects are kept
    rather than discarded -- they were observed, only their height is implausible.
    """
    shelf = _make_shelf(
        relative_heights=(0.3, 1.0),
...
def test_an_object_on_the_shelfs_top_is_not_rejected_for_lack_of_headroom(
    ...
    """
    Nothing stands above the shelf's top, so an object placed there has open air
    over it. Measuring its headroom against the corpus ceiling -- which lies
    *below* the top surface -- makes every such object appear too tall and drops
    the whole layer.
    """
    shelf = _make_shelf(
...
def test_an_object_with_no_mesh_spawns_a_placeholder_when_asked() -> None:
    """
    Objects whose type has no cached mesh are skipped in silence, so a shelf can
    render nearly empty with nothing to say why. Standing a plain box in their
    place makes the gap between what was sampled and what could be shown visible
    while the mesh library is incomplete.
    """
...
def test_objects_with_no_mesh_are_dropped_by_default() -> None:
    """
    Placeholders are a diagnostic, so a generation run that is not being inspected
    must not silently gain boxes that stand for nothing.
    """
    shelf = _make_shelf(relative_heights=(0.3,))
    shelf.source_ids = []
...
def test_only_one_layer_can_occupy_the_shelfs_top() -> None:
    """
    Layers are drawn independently, so several can come back recorded at the
    shelf's top. A shelf has one top, and placing them all there stacks slabs at
    the same height with no room between them for anything to stand.
    """
    shelf = _make_shelf(
...
def test_a_placeholder_can_be_repositioned_like_a_real_object() -> None:
    """
    Placeholders stand in the object list, so collision repair moves them exactly
    as it moves real objects. Attached rigidly they cannot be moved at all, and
    the first repair pass fails on the whole shelf rather than on the stand-in.
    """
    shelf = _make_shelf(relative_heights=(0.3,))
    shelf.source_ids = []
