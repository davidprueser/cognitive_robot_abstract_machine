...
def semantic_annotations_on_surfaces(
    ...
    """
    Queries a list of Semantic annotations that are on top of a given list of
    other annotations (ex.
...
def get_next_object_using_planar_distance(
    ...
    relative to the given main body and supporting surface. This function
    utilizes semantic annotations of objects and orders them by their Euclidean
    distances to the main body.

    :param main_body: The main body to which the Euclidean distance is
        computed.
    :param supporting_surface: The surface on which the semantic
        annotations of interest are queried.
    :return: A `QueryObjectDescriptor` containing semantic annotations
        ordered by Euclidean distance to the main body.
    """
...
def goal_surface_of_object(
    ...
    """
    Finds the most similar object to a given semantic annotation among a list
    of tables based on the inheritance path length. If the similarity does not
    meet the provided threshold, the method attempts to return the table that
    is not supporting any object. The similarity metric leverages the class
    hierarchy to compute distances.

    :param object_of_interest: The semantic annotation to compare.
    :param supporting_surfaces: A list of supporting surfaces semantic
        annotations to search on top of them for similar objects to the
        object_of_interest.
    :param threshold: The maximum acceptable inheritance path length to
        classify objects as similar. Defaults to 1.
    :return: The semantic annotation of the most appropriate surface
        based on similarity metrics or the non-supporting table when no
        viable candidate is found, or None if there are no supporting
        surfaces.
    """
...
def annotation_class_by_label(label: str) -> Optional[type]:
    ...
    :return: The matching class (e.g., Bowl) or None if no match is
        found.
    """
...
def sort_annotations_by_volume(
    ...
    """
    Sorts a list of SemanticAnnotations by volume in descending order (largest.

    to smallest). Volume is calculated by multiplying the scale
    dimensions (x * y * z) of the object's shape.
    ...
    :param order: Whether to sort in ascending or descending order
        (default is True).
    :return: List of SemanticAnnotation objects sorted by volume
        (largest to smallest).
    """
