...
class WorldEntityWithIDKwargsTracker:
    ...
    def from_kwargs(cls, from_json_kwargs) -> Self:
        """
        Retrieve the tracker from the kwargs, or initialize a new one if it
        doesn't exist.

        Adds itself to the kwargs so that it is available for future
        from_json calls.
    ...
    def from_world(cls, world: World) -> Self:
        ...
        :param world: A world instance that will be used as a backup to
            look for world entities.
        """
        if world is None:
    ...
    def create_kwargs(self) -> Dict[str, Self]:
        ...
        The top-level object that calls from_json should add this to its
        kwargs.
    ...
    def add_to_kwargs(self, kwargs: Dict[str, Any]):
        ...
        :param kwargs: A dictionary to which the current instance will
            be added. The specific key is determined by the internal
    ...
    def add_world_entity_with_id(self, world_entity_with_id: WorldEntityWithID):
        """
        Add a new world entity with id to the tracker in-place, to make it
        available for parsing in future from_json calls.
        """
    ...
    def get_world_entity_with_id(self, id: UUID) -> WorldEntityWithID:
        ...
        collection. If the entity is not found and a world object is
        available, it will try to retrieve the entity by its UUID from
        the world object.
