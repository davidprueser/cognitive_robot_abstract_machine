...
class ProcthorResolver:
    """
    Central resolver that deterministically maps a ProcTHOR name to exactly one
    class.
    """
    ...
    def resolve(self, name: str) -> Optional[Type[SemanticAnnotation]]:
        ...
        name by removing numbers and splitting it into tokens, and then
        compares these tokens with the corresponding data in the available
        classes to find the best match.

        :param name: The name to resolve, represented as a string.
        :return: The class with the best match to the given name, or
